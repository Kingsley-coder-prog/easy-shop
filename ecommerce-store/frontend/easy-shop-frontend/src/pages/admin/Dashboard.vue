    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <BaseCard>
        <template #header>
          <div class="flex items-center gap-2">
            <TrendingUpIcon class="w-5 h-5 text-blue-600" />
            <h3 class="text-lg font-semibold text-gray-900">Orders (Last 7 Days)</h3>
          </div>
        </template>
        <div class="h-40 flex items-end gap-2">
          <div v-for="(d, i) in ordersByDay" :key="i" class="flex-1">
      ...
      <BaseCard>
        <template #header>
          <div class="flex items-center gap-2">
            <CurrencyDollarIcon class="w-5 h-5 text-green-600" />
            <h3 class="text-lg font-semibold text-gray-900">Sales (₦) - Last 7 Days</h3>
          </div>
        </template>
        <div class="h-40 flex items-end gap-2">
          <div v-for="(d, i) in salesByDay" :key="i" class="flex-1">
    ...
    <BaseCard>
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="o in recentOrders" :key="o.order_id" class="hover:bg-gray-50 transition">
              <td class="px-6 py-4 font-mono text-xs text-gray-600">
                {{ o.order_id.slice(0, 12) }}...
              </td>
              <td class="px-6 py-4 text-gray-900">
              ...
              <td class="px-6 py-4 font-semibold text-gray-900">₦{{ o.amount_naira }}</td>
              <td class="px-6 py-4">
                <span
                  :class="{
                    'bg-green-100 text-green-800': o.status === 'Paid',
